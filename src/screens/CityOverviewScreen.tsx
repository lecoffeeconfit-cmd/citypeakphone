...
function SourceDetails({ source }: { source: SourceMeta }) {
  ...
  return <View style={{ marginTop: 14, borderTopWidth: 1, borderTopColor: "rgba(186,230,253,0.16)", paddingTop: 10 }}>
    ...
    <Text style={{ color: "#8EA9BC", fontSize: 11, lineHeight: 17, marginTop: 3 }}>Data period: {status.dataPeriodStart && status.dataPeriodEnd ? `${status.dataPeriodStart}–${status.dataPeriodEnd}` : "Current conditions"}</Text>
    <Text style={{ color: "#8EA9BC", fontSize: 11, lineHeight: 17 }}>Source updated: {time(status.sourceLastModifiedAt)}</Text>
...
function Overview({ data }: { data: CityOverviewData }) {
  ...
  return <>
    ...
    <AvailabilityCard title="Air quality" availability="unavailable" description="Air quality will appear when CityPeak’s server-side AirNow connector is configured. No key is stored in this app." />
...
function Safety({ data }: { data: CityOverviewData }) {
  ...
  return <>
    ...
    {crimeSource ? <OfficialLinkCard link={crimeSource} eyebrow="Official crime dashboard" /> : <AvailabilityCard title="Crime trends" availability={data.availability.crime} description="Limited official data available. CityPeak does not infer crime rates, mix reporting periods, or label a city safe or unsafe." />}
    {crimeSource ? <Card eyebrow="About crime data" title="Clear, careful comparisons"><Text style={{ color: "#C5D5E2", lineHeight: 21, marginTop: 8 }}>Reported incidents are not convictions. CityPeak will only show category comparisons after a trusted server sync validates the agency, geographic scope, complete reporting period, population, and coverage. Until then, use the official dashboard directly.</Text></Card> : null}
  </>;
}
...
function Issues({ data, onOpenCommunity }: { data: CityOverviewData; onOpenCommunity: () => void }) {
  ...
  return <>
    {source ? <OfficialLinkCard link={source} eyebrow="Official open-data record" /> : <AvailabilityCard title="Service requests" availability={data.availability.issues} description="Official service-request data is not currently available in CityPeak for this city." />}
...
function Government({ data }: { data: CityOverviewData }) {
  ...
  if (!officials.length) return <AvailabilityCard title="Government directory" availability={data.availability.government} description="Verified records are not currently available for this city. CityPeak only displays officials after manual verification against an official government source." />;
  return <>
    {mayor ? <Card eyebrow="Verified official" title={mayor.name} accent="#65D7B1"><Text style={{ color: "#E9F8FB", fontWeight: "800", marginTop: 7 }}>{mayor.displayedTitle}</Text><Text style={{ color: "#9EB9C9", marginTop: 10, fontSize: 12 }}>Verified against the City of Long Beach directory · {time(mayor.lastVerifiedAt)}</Text><Pressable accessibilityRole="link" onPress={() => openUrl(mayor.officialProfileUrl)} style={{ marginTop: 14 }}><Text style={{ color: "#77D8F4", fontWeight: "900" }}>Official profile and contact ↗</Text></Pressable></Card> : null}
    <Card eyebrow="Verified directory" title="City Council" accent="#65D7B1"><View style={{ marginTop: 10, gap: 0 }}>{council.map((official) => <Pressable key={official.id} accessibilityRole="link" accessibilityLabel={`Open official profile for ${official.name}`} onPress={() => openUrl(official.officialProfileUrl)} style={{ flexDirection: "row", justifyContent: "space-between", gap: 10, paddingVertical: 11, borderBottomWidth: 1, borderBottomColor: "rgba(186,230,253,0.12)" }}><View style={{ flex: 1 }}><Text style={{ color: "#F6FCFF", fontWeight: "800" }}>{official.name}</Text><Text style={{ color: "#9EB9C9", marginTop: 2, fontSize: 12 }}>{official.district || official.displayedTitle}</Text></View><Text style={{ color: "#77D8F4", fontWeight: "900" }}>View ↗</Text></Pressable>)}</View></Card>
